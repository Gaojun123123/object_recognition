...
void USART2_IRQHandler(void)
{
	...
	{	 
		res=USART2->DR; 			 
		if((USART2_RX_STA&0x8000)==0)		//接受未完成
		{
			if(USART2_RX_STA&0x4000)  //接受到了0x0d
			{
				if(res!=0x0a)USART2_RX_STA=0;//接受错误，重新开始
				else{
					//接受完成了
					USART2_RX_STA|=0x8000;//接受完成了
			}
		...
		}else //还没收到0x0d
			{
				if(res==0x0d)USART2_RX_STA|=0x4000;
					...
					USART2_RX_STA++;
					if(USART2_RX_STA>(USART2_RX_EN-1))USART2_RX_STA=0;
					}
			}
	}  											 
}   
...
{  	 		 
	...
	GPIOA->CRL&=0XFFFF00FF;	//IO状态设置 pA10 11
